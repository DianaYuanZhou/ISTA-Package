...
# library(MASS) library(Metrics) source('Function.R')

ISTA.main <- function(data.X, data.Y, scale = T, lambda = 0.01, 
    method = NULL, epoch = 10000, patience = 50) {
    
    stopifnot(!missing(data.X), !missing(data.Y), is.matrix(data.X), 
        is.matrix(data.Y), ncol(data.Y) == 1)
    ...
    list2env(split(mget(c("data.X", "data.Y")), 0.8, "train", 
        "test"), envir = environment())
    ISTA.result <- list()
    
    if (is.null(method)) {
        for (i in 1:length(lambda)) {
            parameters.best <- ISTA0(data.X.train, data.Y.train, 
                data.X.test, data.Y.test, epoch = epoch, patience = patience, 
                lambda = lambda[i])$parameters.best
            Y.test.hat <- pred(data.Y.test, data.X.test, parameters.best)$Y.test.hat
            test.mse <- mse(data.Y.test, Y.test.hat)
            cat("Test MSE in test set with lambda", lambda[i], 
                "is", test.mse, "\n")
        }
        ISTA.result <- list(`Estimate coefficient` = parameters.best, 
            `MSE in test set` = test.mse, Prediction = Y.test.hat)
        print(parameters.best)
        return(ISTA.result)
    } else {
        stopifnot(method %in% c("CV", "AIC", "BIC"))
        p <- ncol(data.X)
        result.list <- list()
        # --------- Split datasets and find best labmda ------------
        if ("CV" %in% method) {
            fold = 10
            result.cv <- data.frame(lambda = seq(0, 0, len = length(lambda)), 
                valid.error.mean = seq(0, 0, len = length(lambda)), 
                epoch = seq(0, 0, len = length(lambda)))
            valid.error.cv <- numeric(fold)
            training.epoch <- numeric(fold)
            for (i in 1:length(lambda)) {
                data.train <- data.frame(data.X.train, data.Y.train)
                data.train.s <- split.data.frame(data.train, factor(1:fold))
                for (j in 1:fold) {
                  ...
                  training.result <- ISTA0(data.X.subtrain, data.Y.subtrain, 
                    data.X.valid, data.Y.valid, epoch = epoch, 
        ...
        if ("AIC" %in% method || "BIC" %in% method) {
            list2env(split(list(data.X = data.X.train, data.Y = data.Y.train), 
                0.75, "subtrain", "valid"), envir = environment())
            result.aic <- data.frame(lambda = seq(0, 0, len = length(lambda)), 
                AIC = seq(0, 0, len = length(lambda)), epoch = seq(0, 
                  0, len = length(lambda)))
            result.bic <- data.frame(lambda = seq(0, 0, len = length(lambda)), 
                BIC = seq(0, 0, len = length(lambda)), epoch = seq(0, 
                  0, len = length(lambda)))
            
            for (i in 1:length(lambda)) {
                training.result <- ISTA0(data.X.subtrain, data.Y.subtrain, 
                  data.X.valid, data.Y.valid, epoch = epoch, patience = patience, 
                  lambda = lambda[i])
                parameters.best <- training.result$parameters.best
                
                if ("AIC" %in% method) {
                  aic <- tcrossprod(t(data.Y.valid - data.X.valid %*% 
                    parameters.best))/(nrow(data.X.valid) * var(data.Y.valid)) + 
                    2 * (length(which(parameters.best == 0)))/nrow(data.X.valid)
                ...
                if ("BIC" %in% method) {
                  bic <- tcrossprod(t(data.Y.valid - data.X.valid %*% 
                    parameters.best))/(nrow(data.X.valid) * var(data.Y.valid)) + 
                    log(nrow(data.X.valid)) * (length(which(parameters.best == 
                      0)))/nrow(data.X.valid)
            ...
            if ("AIC" %in% method) {
                cat("Best lambda under AIC criteria is", result.aic[which.min(result.aic$AIC), 
                  "lambda"], "\n")
                result.list[["AIC"]] <- result.aic
            }
            if ("BIC" %in% method) {
                cat("Best lambda under BIC criteria is", result.bic[which.min(result.bic$BIC), 
                  "lambda"], "\n")
        ...
        for (med in method) {
            ...
            lambda.best <- result[which.min(result[, 2]), "lambda"]
            parameters.best <- ISTA0(data.X.subtrain, data.Y.subtrain, 
                data.X.valid, data.Y.valid, epoch = epoch, patience = patience, 
                lambda = lambda.best)$parameters.best
            Y.test.hat <- pred(data.Y.test, data.X.test, parameters.best)$Y.test.hat
            test.mse <- mse(data.Y.test, Y.test.hat)
