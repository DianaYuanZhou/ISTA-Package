...
Obj.func <- function(Y, X, beta, step_size) {
    g.value <- tcrossprod(t(Y - X %*% beta))/(2 * step_size)
    # g.null <- tcrossprod(t(Y- mean(Y)))/(2*step_size)
    return(g.value)
}
...
Grad <- function(Y, X, parameters, lambda, step_size) {
    ...
    grad.g <- (parameters - S(lambda, X = parameters - step_size * 
        grad))/step_size
...
backtracking <- function(Y, X, parameters, grad, grad.g, lambda, 
    step_size, constant) {
    # list2env(Grads, envir = environment()) list2env(grad, envir
    # = environment()) list2env(grad.g, envir = environment())
    
    while (Obj.func(Y, X, beta = parameters - step_size * grad.g, 
...
# Update parameters
update.parameters <- function(lambda, grad, parameters, step_size) {
    parameters_new = S(lambda, parameters - step_size * grad)
...
pred <- function(Y.test, X.test, parameters) {
    ...
    test.error <- Obj.func(Y.test, X.test, parameters, step_size = 1)
    return(mget(c("Y.test.hat", "test.error")))
}

# Main algrithm
ISTA0 <- function(X.subtrain, Y.subtrain, X.valid, Y.valid, epoch = 10000, 
    patience, lambda) {
    # --------- Initializa parameter ---------
    p <- ncol(X.subtrain)
    parameters <- runif(p, -0.5, 0.5)
    step_size <- 0.001
    valid.error.best <- Obj.func(Y.valid, X.valid, parameters, 
        step_size)
    ...
    # -----------
    for (i in 1:epoch) {
        Grads <- Grad(Y.subtrain, X.subtrain, parameters, lambda = lambda, 
            step_size)
        step_size_update <- backtracking(Y.subtrain, X.subtrain, 
            Grads$grad, Grads$grad.g, parameters, lambda = lambda, 
            step_size, constant = 1/2)
        parameters <- update.parameters(lambda = lambda, Grads$grad, 
            parameters, step_size_update)
        valid.error <- Obj.func(Y.valid, X.valid, parameters, 
            step_size)
